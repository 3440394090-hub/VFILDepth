...
class scale_casa_HAM(nn.Module):
    def __init__(self, num_channel):
        ...
        self.casa_0 = CASA_HAM(in_channel=num_channel[0])
        self.casa_1 = CASA_HAM(in_channel=num_channel[1])
        self.casa_2 = CASA_HAM(in_channel=num_channel[2])
        self.casa_3 = CASA_HAM(in_channel=num_channel[3])
        ...
        self.Down_ch = torch.nn.Sequential(
            ...
            nn.ELU())
        '''
        self.f0_down = nn.AvgPool2d(kernel_size=2, stride=2)


        self.elu = nn.ELU()

    def forward(self, feature):
        # 假设输入 feature 分辨率为 [1/2, 1/4, 1/8, 1/16]
        refined_features = []

        # 1. 提取并增强特征
        f0 = self.casa_0(feature[0])
        refined_features.append(f0)

        f1 = self.casa_1(feature[1])
        refined_features.append(f1)

        f2 = self.casa_2(feature[2])
        refined_features.append(f2)

        f3 = self.casa_3(feature[3])
        refined_features.append(f3)
        ...
        # f2: 1/8 -> 1/4 (上采样 2 倍)
        f2_aligned = F.interpolate(f2, scale_factor=2, mode='bilinear', align_corners=True)

        # f3: 1/16 -> 1/4 (上采样 4 倍)
        f3_aligned = F.interpolate(f3, scale_factor=4, mode='bilinear', align_corners=True)

        # 3. 拼接与全局融合
        f_concat = torch.cat([f0_aligned, f1_aligned, f2_aligned, f3_aligned], dim=1)
        f_global = self.Down_ch(f_concat)

        # 将融合后的全局特征插入到首位，最终返回长度为 5 的列表
        refined_features.insert(0, f_global)
