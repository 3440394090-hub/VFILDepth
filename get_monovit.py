...
def get_monovit_pretrained(load_type):
    ...
    if load_type == 1024:
        depth_dict = torch.load(r"D:\Root\rd_F\mobilevit_distilla_pose_nofreeze_light_decoder\models\monovit_1024\depth.pth")
        # new_dict = depth_dict
        # 调整预训练模型的权重字典
        new_dict = {}
    ...
    # if load_type == 640:
    #     # 加载编码器的权重
    #     mpvit_encoder_dict = torch.load(r"D:\Root\rd_F\mobilevit_distilla_pose_nofreeze_light_decoder\models\monovit640_stereo\encoder.pth")
    #     model_dict = depth.encoder.state_dict()
    #     depth.encoder.load_state_dict({k: v for k, v in mpvit_encoder_dict.items() if k in model_dict})
    #     # 加载解码器的权重
    #     mpvit_decoder_path = r"D:\Root\rd_F\mobilevit_distilla_pose_nofreeze_light_decoder\models\monovit640_stereo\depth.pth"
    #     depth.decoder.load_state_dict(torch.load(mpvit_decoder_path))
